// v0.62.5 ：StrategyWizard — 抽出来的 4 阶段 wizard 控件
//   goal → input → directions → loading-expand
//
// Composer 板块 (/composer) 默认渲染这个，用户从「选起号目标」开始
// 走完 4 步生成 pack。完成后 onPackReady(pack_id) 触发 Composer 继续
// 进入「写每篇」环节。
//
// Strategy 板块 (/strategy/:pack_id) 不再使用这个 — 它只用 StrategyPackView 看大纲。
...
/** Wizard 完成 ：用户成功 expand 出一份 pack。Composer 接住这个回调
 *  进入「写每篇」环节。Strategy 板块（/strategy/:pack_id）不调用 wizard，
 *  所以这个 prop 在那里不需要。 */
export interface StrategyWizardProps {
  onPackReady: (packId: string) => void;
}

export default function StrategyWizard({ onPackReady }: StrategyWizardProps) {
  // v0.62.5 ：wizard 不再支持「直接打开某个 pack」入口（那个走 /strategy/:pack_id）。
  // wizard 永远从 goal phase 开始，完成后回调宿主导航。
  const urlPackId: string | undefined = undefined;
    ...
    <div>
      ...
      {phase === "pack" && pack && (
        <div className="card" style={{textAlign: "center", padding: 32}}>
          ...
          <h2 style={{margin: 0}}>策略已就绪 — 进入「写每篇」</h2>
          <p className="muted" style={{fontSize: 13, marginTop: 6}}>
            排期完成。下面会列出所有 slot，选哪条多 agent 团队就写哪条。
          </p>
          <button onClick={() => onPackReady(pack.pack_id)}
