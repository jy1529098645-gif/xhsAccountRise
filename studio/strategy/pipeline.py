...
def _build_phase_rules(cycle_weeks: int) -> str:
    ...
    if cycle_weeks == 4:
        return (
            ...
            "  · 第 1 周（intent=「拉新 + 立人设」严格 — 最重要的一周）：\n"
            "      - hook_type 只能选 数字型 / 痛点型 / 故事型 / 段子型\n"
            "      - **严禁挂产品链接 / 严禁「我推荐 XX」/ 严禁「试试 XX」类卖货语句**\n"
            "      - **产品/工具/品牌只能在最多 10% slot 极弱出现**（如 1 篇 7 步法里第 3 步顺带提一句「我用 XX」），其余 90% 完全没有产品\n"
            "      - 100% slot 必须给真心干货 / 痛点共鸣 / 真实经历，让陌生用户觉得「这人懂我，先关注再说」\n"
            "      - 必须 ≥ 50% slot 直接回应 DNA「用户高频询问」的具体痛点（用你自己的话表达，禁止逐字搬评论）\n"
            "      - ⚠️ 这一周如果不立人设直接卖货，账号没救（算法判低质 + 用户判微商号秒划走 = 立刻入死号池）\n"
            "      - 目的 ：建立「我懂你」信任 + 个人 IP，**完全不卖产品**\n"
    ...
    if cycle_weeks <= 8:
        return (
            ...
            f"  · 第 1-{max(2, cycle_weeks // 4)} 周（拉新）：回应 DNA 评论里的具体痛点（抓 pain point 用你自己的话回应，别逐字复制评论），禁挂链接\n"
    ...
    return (
        ...
        f"  · 第 1-{q} 周（拉新）：回应 DNA 评论里的具体痛点（抓 pain point 用你自己的话回应，别逐字复制评论）\n"
...
async def _expand_inner(
    ...
    sched_user = (
        ...
        + ("\n\n⭐ **产品上下文使用规则**（区分必须 verbatim vs 必须重写）：\n"
           "  ✅ **必须 verbatim（精确复制，禁止改）**：\n"
           "      · 产品 / 工具 / 品牌名（不能编造或改名）\n"
           "      · 具体数字 / 数据（不能换算或近似）\n"
           "      · 平台红线词 / 禁忌词（不能挑战合规）\n"
           "  🔁 **必须重写（避免重复 spam）**：\n"
           "      · 核心叙事金句 / 场景金句 — 30 篇里同一句金句最多用 1-2 次\n"
           "      · 每篇必须用不同表达 / 句式 / 切入角度，体现同一立意\n"
           "      · 例 ：「以前要开 8 个网页」可以变体成「之前 5 个标签页轮转」/「开一堆 tabs 真的崩」/「换工具像换 tab 一样累」 ——核心信息一致（多工具切换痛点），表达完全不同\n"
           "  📐 **覆盖率 — phase-aware，绝对禁止热启动卖货**：\n"
           "      · 第 1 周（拉新期）：**≤ 10% slot** 可隐性提产品（且不挂链接）。90% slot 100% 立人设 / 痛点共鸣 / 真心干货，**绝对禁止「我用了 XX」「我推荐 XX」类直白卖货语句**。这一周用户对你完全陌生，强卖货 = 立刻划走 + 算法降权。\n"
           "      · 第 2 周（专业感期）：**≤ 30% slot** 可作为「我自己用的工具」**隐性露出**（不是推荐）。70% slot 继续深度方法论 / 工具流 / 干货，建立「这人是真懂行的」。\n"
           "      · 第 3 周（沉淀期）：**40-50% slot** 可回到产品 + 系列化（N 步法 / 多场景拆解 / 用户证言）。\n"
           "      · 第 4 周（转化期）：**70%+ slot** 强卖点 + 评论引导 + CTA。可挂主页、可引私域。这是收割期。\n"
           "      · ⚠️ 一上来就 50% 全推产品 = 拉新期没立人设就强卖货 = 死号路径。**严格按周期递增产品权重。**"
           if pctx else "")
