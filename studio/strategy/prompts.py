...
**重要**：这一步只输出结构（标题/大纲/材料/时段），**不要写正文**。正文会在下一步由专门的写手并行写。
...
BODY_DRAFTER_SYSTEM = """\
你是「起号文案手」。给你一个已经排好的内容 slot（标题 + 大纲 + 角度 + 平台 + 受众），写一份**可发布的初稿正文**。

要求：
- 长度 ：300-600 字（小红书/抖音文案；B站/Reddit/X 按对应平台习惯）
- 开头第一行就是 hook（钩子句），不要客套
- 中段按 outline 展开，每段一个明确主张 + 例子或数据点
- 结尾给一个行动指令或互动钩子（评论引导 / 收藏理由 / 私信触发）
- 平台适配 ：小红书加 emoji + 段落分隔 ；抖音脚本化（[镜头 1] / [口播]）；B站长一点；Reddit/X 用英文 markdown
